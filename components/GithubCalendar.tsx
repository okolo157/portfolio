...
const GitHubStats = ({ username, since, until }: { username: string; since?: string; until?: string }) => {
    const [stats, setStats] = useState<GitHubStats | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const fetchAndSetStats = async (username: string, since?: string, until?: string) => {
        try {
            setLoading(true);
            setError(null);
            const fetchedStats = await fetchGithubStats(username, since, until);
    ...
    useEffect(() => {
        if (username) {
            fetchAndSetStats(username, since, until);
        }
